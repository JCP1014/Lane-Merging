...
def window_oneSol_dp_v1(a, b, c, W_same, W_diff, last_X, last_Y, keep):
    t0 = time.time()
    alpha = len(a) - 1
    beta = len(b) - 1
    gamma = len(c) - 1
    ...
    if opt.time == L_AB[i][j][k].time:
        ...
        if lanes == 'XY':
            ...
            if table == 'AB':
                stack_X.append(
                    ('B', j-1, max(b[j-1], L_AB[i][j-2][k].time[0]+W_diff)))
            elif table == 'BB':
                stack_X.append(
...
def window_oneSol_dp_v2(a, b, c, W_same, W_diff, last_X, last_Y, keep):
    t0 = time.time()
    alpha = len(a) - 1
    beta = len(b) - 1
    gamma = len(c) - 1
    ...
    for i in range(1, alpha+1):
        for j in range(1, beta+1):
            for k in range(1, gamma+1):
                L_AB[i][j][k] = min(
                    Sol((max(a[i], L_AB[i-1][j][k].time[0]+W_same), L_AB[i-1][j][k].time[1]), 'AB', 0, 'X'),
                    Sol((max(a[i], L_BB[i-1][j][k].time[0]+W_diff), L_BB[i-1][j][k].time[1]), 'BB', 0, 'X'),
                    Sol((L_AB[i][j-1][k].time[0], max(b[j], L_AB[i][j-1][k].time[1]+W_same)), 'AB', 0, 'Y'),
                    Sol((L_AC[i][j-1][k].time[0], max(b[j], L_AC[i][j-1][k].time[1]+W_diff)), 'AC', 0, 'Y'),
                    key=get_obj
                )
                L_AC[i][j][k] = min(
                    Sol((max(a[i], L_AC[i-1][j][k].time[0]+W_same), L_AC[i-1][j][k].time[1]), 'AC', 0, 'X'),
                    Sol((max(a[i], L_BC[i-1][j][k].time[0]+W_diff), L_BC[i-1][j][k].time[1]), 'BC', 0, 'X'),
                    Sol((L_AC[i][j][k-1].time[0], max(c[k], L_AC[i][j][k-1].time[1]+W_same)), 'AC', 0, 'Y'),
                    Sol((L_AB[i][j][k-1].time[0], max(c[k], L_AB[i][j][k-1].time[1]+W_diff)), 'AB', 0, 'Y'),
                    key=get_obj
                )
                L_BC[i][j][k] = min(
                    Sol((max(b[j], L_BC[i][j-1][k].time[0]+W_same), L_BC[i][j-1][k].time[1]), 'BC', 0, 'X'),
                    Sol((max(b[j], L_AC[i][j-1][k].time[0]+W_diff), L_AC[i][j-1][k].time[1]), 'AC', 0, 'X'),
                    Sol((L_BC[i][j][k-1].time[0], max(c[k], L_BC[i][j][k-1].time[1]+W_same)), 'BC', 0, 'Y'),
                    Sol((L_BB[i][j][k-1].time[0], max(c[k], L_BB[i][j][k-1].time[1]+W_diff)), 'BB', 0, 'Y'),
                    key=get_obj
                )
                L_BB[i][j][k] = min(
                    Sol((max(b[j], L_BB[i][j-1][k].time[0]+W_same), L_BB[i][j-1][k].time[1]), 'BB', 0, 'X'),
                    Sol((max(b[j], L_AB[i][j-1][k].time[0]+W_diff), L_AB[i][j-1][k].time[1]), 'AB', 0, 'X'),
                    Sol((L_BB[i][j-1][k].time[0], max(b[j], L_BB[i][j-1][k].time[1]+W_same)), 'BB', 0, 'Y'),
                    Sol((L_BC[i][j-1][k].time[0], max(b[j], L_BC[i][j-1][k].time[1]+W_diff)), 'BC', 0, 'Y'),
                    key=get_obj
                )
...
def window_allSol_dp_v1(a, b, c, W_same, W_diff, last_X, last_Y, keep):
    t0 = time.time()
...
def window_allSol_dp_v2(a, b, c, W_same, W_diff, last_X, last_Y, keep):
    t0 = time.time()
    alpha = len(a) - 1  # Number of vehicles in lane A
    beta = len(b) - 1   # Number of vehicles in lane B
    gamma = len(c) - 1  # Number of vehicles in lane C
    ...
    for i in range(1, alpha+1):
        for j in range(1, beta+1):
            for k in range(1, gamma+1):
                L_AB[i][j][k] = [Sol((max(a[i], s.time[0]+W_same), s.time[1]), 'AB', idx, 'X') for idx, s in enumerate(L_AB[i-1][j][k]) if not float('inf') in s.time] + \
                                [Sol((max(a[i], s.time[0]+W_diff), s.time[1]), 'BB', idx, 'X') for idx, s in enumerate(L_BB[i-1][j][k]) if not float('inf') in s.time] + \
                                [Sol((s.time[0], max(b[j], s.time[1]+W_same)), 'AB', idx, 'Y') for idx, s in enumerate(L_AB[i][j-1][k]) if not float('inf') in s.time] + \
                                [Sol((s.time[0], max(b[j], s.time[1]+W_diff)), 'AC', idx, 'Y') for idx, s in enumerate(L_AC[i][j-1][k]) if not float('inf') in s.time]

                L_AC[i][j][k] = [Sol((max(a[i], s.time[0]+W_same), s.time[1]), 'AC', idx, 'X') for idx, s in enumerate(L_AC[i-1][j][k]) if not float('inf') in s.time] + \
                                [Sol((max(a[i], s.time[0]+W_diff), s.time[1]), 'BC', idx, 'X') for idx, s in enumerate(L_BC[i-1][j][k]) if not float('inf') in s.time] + \
                                [Sol((s.time[0], max(c[k], s.time[1]+W_same)), 'AC', idx, 'Y') for idx, s in enumerate(L_AC[i][j][k-1]) if not float('inf') in s.time] + \
                                [Sol((s.time[0], max(c[k], s.time[1]+W_diff)), 'AB', idx, 'Y') for idx, s in enumerate(L_AB[i][j][k-1]) if not float('inf') in s.time]

                L_BC[i][j][k] = [Sol((max(b[j], s.time[0]+W_same), s.time[1]), 'BC', idx, 'X') for idx, s in enumerate(L_BC[i][j-1][k]) if not float('inf') in s.time] + \
                                [Sol((max(b[j], s.time[0]+W_diff), s.time[1]), 'AC', idx, 'X') for idx, s in enumerate(L_AC[i][j-1][k]) if not float('inf') in s.time] + \
                                [Sol((s.time[0], max(c[k], s.time[1]+W_same)), 'BC', idx, 'Y') for idx, s in enumerate(L_BC[i][j][k-1]) if not float('inf') in s.time] + \
                                [Sol((s.time[0], max(c[k], s.time[1]+W_diff)), 'BB', idx, 'Y') for idx, s in enumerate(L_BB[i][j][k-1]) if not float('inf') in s.time]

                L_BB[i][j][k] = [Sol((max(b[j], s.time[0]+W_same), s.time[1]), 'BB', idx, 'X') for idx, s in enumerate(L_BB[i][j-1][k]) if not float('inf') in s.time] + \
                                [Sol((max(b[j], s.time[0]+W_diff), s.time[1]), 'AB', idx, 'X') for idx, s in enumerate(L_AB[i][j-1][k]) if not float('inf') in s.time] + \
                                [Sol((s.time[0], max(b[j], s.time[1]+W_same)), 'BB', idx, 'Y') for idx, s in enumerate(L_BB[i][j-1][k]) if not float('inf') in s.time] + \
                                [Sol((s.time[0], max(b[j], s.time[1]+W_diff)), 'BC', idx, 'Y') for idx, s in enumerate(L_BC[i][j-1][k]) if not float('inf') in s.time]
...
def schedule_by_num_window_v1(a_all, b_all, c_all, W_same, W_diff, carNum, keep, allSol):
    last_X = ('', 0, 0.0)
    last_Y = ('', 0, 0.0)
    if allSol:
        t0 = time.time()
        while len(a_all) > 1 or len(b_all) > 1 or len(c_all) > 1:
            ...
            if len(a) > 1 and len(b) > 1 and len(c) > 1:
                last_X, last_Y, _ = window_allSol_dp_v1(
                    a, b, c, W_same, W_diff, last_X, last_Y, keep)
        ...
        while len(a_all) > 1 or len(b_all) > 1 or len(c_all) > 1:
            ...
            c, c_all = get_window_by_num(c_all, carNum, keep)
            if len(a) > 1 and len(b) > 1 and len(c) > 1:
                last_X, last_Y, _ = window_oneSol_dp_v1(
                    a, b, c, W_same, W_diff, last_X, last_Y, keep)
            elif len(a) > 1 and len(b) > 1:
                last_X = schedule_single_lane('A', a, W_same, W_diff, last_X)
                last_Y = schedule_single_lane('B', b, W_same, W_diff, last_Y)
...
def schedule_by_num_window_v2(a_all, b_all, c_all, W_same, W_diff, carNum, keep, allSol):
    last_X = ('', 0, 0.0)
    last_Y = ('', 0, 0.0)
    if allSol:
        t0 = time.time()
        ...
        while len(a_all) > 1 or len(b_all) > 1 or len(c_all) > 1:
            ...
            c, c_all = get_window_by_num(c_all, carNum, keep)
            if len(a) > 1 and len(b) > 1 and len(c) > 1:
                last_X, last_Y, _ = window_allSol_dp_v2(
                    a, b, c, W_same, W_diff, last_X, last_Y, keep)
            elif len(a) > 1 and len(b) > 1:
                last_X = schedule_single_lane('A', a, W_same, W_diff, last_X)
                last_Y = schedule_single_lane('B', b, W_same, W_diff, last_Y)
        ...
        while len(a_all) > 1 or len(b_all) > 1 or len(c_all) > 1:
            ...
            c, c_all = get_window_by_num(c_all, carNum, keep)
            if len(a) > 1 and len(b) > 1 and len(c) > 1:
                last_X, last_Y, _ = window_oneSol_dp_v2(
                    a, b, c, W_same, W_diff, last_X, last_Y, keep)
            elif len(a) > 1 and len(b) > 1:
                last_X = schedule_single_lane('A', a, W_same, W_diff, last_X)
                last_Y = schedule_single_lane('B', b, W_same, W_diff, last_Y)
...
def main():
    ...
    print(schedule_by_num_window_v1(copy.deepcopy(a_all), copy.deepcopy(
        b_all), copy.deepcopy(c_all), W_same, W_diff, 5, 0, False))
    print(schedule_by_num_window_v1(copy.deepcopy(a_all), copy.deepcopy(
        b_all), copy.deepcopy(c_all), W_same, W_diff, 10, 0, False))
    # print(schedule_by_num_window_v1(copy.deepcopy(a_all), copy.deepcopy(
    #     b_all), copy.deepcopy(c_all), W_same, W_diff, 20, 0, False))
    print('-------------------------------------------All Solutions---------------------------------------------')
    print(schedule_by_num_window_v1(copy.deepcopy(a_all), copy.deepcopy(
        b_all), copy.deepcopy(c_all), W_same, W_diff, 5, 0, True))
    print(schedule_by_num_window_v1(copy.deepcopy(a_all), copy.deepcopy(
        b_all), copy.deepcopy(c_all), W_same, W_diff, 10, 0, True))
    # print(schedule_by_num_window_v1(copy.deepcopy(a_all), copy.deepcopy(
    #     b_all), copy.deepcopy(c_all), W_same, W_diff, 20, 0, True))

    print('-------------------------------------------Version 2---------------------------------------------')
    print(schedule_by_num_window_v2(copy.deepcopy(a_all), copy.deepcopy(
        b_all), copy.deepcopy(c_all), W_same, W_diff, 5, 0, False))
    print(schedule_by_num_window_v2(copy.deepcopy(a_all), copy.deepcopy(
        b_all), copy.deepcopy(c_all), W_same, W_diff, 10, 0, False))
    # print(schedule_by_num_window_v2(copy.deepcopy(a_all), copy.deepcopy(
